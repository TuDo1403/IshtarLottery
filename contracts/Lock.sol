...
contract LottoContract is Ownable {
    ...
    function openSeri(
        ...
        serie memory seri_ = series[_seri];

        require(seri_.price == 0, "Seri existed");
        require(_postPrice <= _price, "Invalid post price");
        seri_.seriType = 1;

        uint256 _currentCarryOverSeri = currentCarryoverSeri;
        if (_initPrize > 0) {
            require(_currentCarryOverSeri == 0 || series[_currentCarryOverSeri].status != 0, "Carry-over seri opening");
            require(initCarryOverAsset.transferFrom(msg.sender, address(this), _initPrize), "Insufficient-allowance");
            seri_.seriType = 2;
            seri_.initPrize = _initPrize;
            _currentCarryOverSeri = _seri;
        }
        seri_.price = _price;
        seri_.max2sale = _max2sale;
        series[_seri] = seri_;

        seriExpiredPeriod[_seri] = expiredPeriod;
        postPrices[_seri] = _postPrice;
        emit OpenSeri(_seri, seri_.seriType);
    }
    ...
    function takePrizeExpired(uint256 _seri) public onlyOwner {
        seri memory seri_ = series[_seri];
        require(!seri_.takeAssetExpired, "Taked");
        require(seri_.endTime.add(seriExpiredPeriod[_seri]) < now, "Ticket not expired");
